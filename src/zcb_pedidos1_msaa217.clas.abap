class ZCB_PEDIDOS1_MSAA217 definition
  public
...
public section.
  ...
  methods CREATE_PERSISTENT
    ...
    returning
      value(RESULT) type ref to ZCL_PEDIDOS1_MSAA217
    raising
...
protected section.
  ...
  types TYP_OBJECT_REF type ref to ZCL_PEDIDOS1_MSAA217 .
  types:
...
CLASS ZCB_PEDIDOS1_MSAA217 IMPLEMENTATION.
...
************************************************************************
  ...
  data: NEW_OBJECT type ref to ZCL_PEDIDOS1_MSAA217.
...
************************************************************************
  ...
  data: THE_OBJECT type ref to ZCL_PEDIDOS1_MSAA217,
...
*<  Generated from mapping:
  ...
  THE_OBJECT->UZEIT = I_OBJECT_DATA-UZEIT.
...
* * ZPED_MSAA217: internal tables for mass operation
  data DB_ITB_ZPED_MSAA217 type STANDARD TABLE OF ZPED_MSAA217.
  data DB_UTB_ZPED_MSAA217 type STANDARD TABLE OF ZPED_MSAA217.
  data DB_DTB_ZPED_MSAA217 type STANDARD TABLE OF ZPED_MSAA217.

* * ZPED_MSAA217: headlines for tables
  data DB_ILN_ZPED_MSAA217 type ZPED_MSAA217.
  data DB_ULN_ZPED_MSAA217 type ZPED_MSAA217.
  data DB_DLN_ZPED_MSAA217 type ZPED_MSAA217.
...
*< Generated from mapping:
   ...
   DB_ILN_ZPED_MSAA217-ERDAT = <FS_INSERT>-ERDAT.
   DB_ILN_ZPED_MSAA217-UZEIT = <FS_INSERT>-UZEIT.
   DB_ILN_ZPED_MSAA217-ERNAM = <FS_INSERT>-ERNAM.


   DB_ILN_ZPED_MSAA217-VBELN = <FS_INSERT>-VBELN.
   append DB_ILN_ZPED_MSAA217 to DB_ITB_ZPED_MSAA217.
*>
...
*< Generated from mapping:
   ...
   DB_ULN_ZPED_MSAA217-ERDAT = <FS_UPDATE>-ERDAT.
   DB_ULN_ZPED_MSAA217-UZEIT = <FS_UPDATE>-UZEIT.
   DB_ULN_ZPED_MSAA217-ERNAM = <FS_UPDATE>-ERNAM.


   DB_ULN_ZPED_MSAA217-VBELN = <FS_UPDATE>-VBELN.
   append DB_ULN_ZPED_MSAA217 TO DB_UTB_ZPED_MSAA217.
*>
...
* * Collect Deletes
  loop at I_DELETES assigning <FS_DELETE>.
    ...
    DB_DLN_ZPED_MSAA217-VBELN = <FS_DELETE>-BUSINESS_KEY-VBELN.

    append DB_DLN_ZPED_MSAA217 TO DB_DTB_ZPED_MSAA217.
...
* * DB Deletes
  delete ZPED_MSAA217 FROM TABLE DB_DTB_ZPED_MSAA217.
  if SY-SUBRC <> 0.
...
* * DB Inserts
  insert ZPED_MSAA217 FROM TABLE DB_ITB_ZPED_MSAA217
...
* * DB Updates
  update ZPED_MSAA217 FROM TABLE DB_UTB_ZPED_MSAA217.
  if SY-SUBRC <> 0.
...
*< Generated from mapping:
  types: TYP_DB_DATA_LOCAL type ZPED_MSAA217.
*>
...
*< Generated from mapping:
  field-symbols: <FS_DB_ZPED_MSAA217> type ZPED_MSAA217.
*>

*< Generated from mapping:
  select * from ZPED_MSAA217
           into table DB_DATA_LOCAL_TAB
           for all entries in I_BUSINESS_KEY_TAB
           where ZPED_MSAA217~VBELN = I_BUSINESS_KEY_TAB-VBELN.
*>
...
* * error handling
  if ( SY-SUBRC <> 0 ).
    ...
    raise exception type CX_OS_DB_SELECT
         exporting TABLE  = 'ZPED_MSAA217'
                   TEXTID = CX_OS_DB_SELECT=>BY_BKEYTAB.
  endif. "( Error )

  loop at DB_DATA_LOCAL_TAB assigning <FS_DB_DATA_LOCAL>.

    assign <FS_DB_DATA_LOCAL> to <FS_DB_ZPED_MSAA217>.

*< Generated from mapping:
    DB_DATA-VBELN = <FS_DB_ZPED_MSAA217>-VBELN.
    DB_DATA-ERDAT = <FS_DB_ZPED_MSAA217>-ERDAT.
    DB_DATA-UZEIT = <FS_DB_ZPED_MSAA217>-UZEIT.
    DB_DATA-ERNAM = <FS_DB_ZPED_MSAA217>-ERNAM.
*>
...
************************************************************************
  ...
  data: OBJECT_IREF type TYP_OBJECT_IREF,
        THE_OBJECT  type ref TO ZCL_PEDIDOS1_MSAA217.
...
*<  Generated from mapping:
    ...
    OBJECT_DATA_ITEM-UZEIT = THE_OBJECT->UZEIT.
...
*      RETURNING RESULT TYPE REF TO ZCL_PEDIDOS1_MSAA217
*       raising   CX_OS_OBJECT_NOT_FOUND
************************************************************************
