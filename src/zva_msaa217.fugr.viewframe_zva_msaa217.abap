*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVA_MSAA217
*   generation date: 23.06.2022 at 11:57:29
*   view maintenance generator version: #001407#
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVA_MSAA217         .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVA_MSAA217'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVA_MSAA217_EXTRACT
TOTAL = ZVA_MSAA217_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVA_MSAA217'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVA_MSAA217-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVA_MSAA217_EXTRACT
TOTAL = ZVA_MSAA217_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVA_MSAA217-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVA_MSAA217'
...
TOTAL = ZVA_MSAA217_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVA_MSAA217-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVA_MSAA217-UPD_FLAG NE SPACE.
